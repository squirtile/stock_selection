...
def print_step_elapsed(step_name: str, start_time: float):
    ...
    print(f"[用时] {step_name}：{elapsed:.2f} 秒")
    return elapsed
...
def evaluate_strategy_from_hist(hist_df: pd.DataFrame):
    """
    对追加了实时动态日K后的历史数据执行已注册日线策略判断。

    这里不再手写每个策略判断，而是复用 strategy.evaluate_latest_signal，
    保证盘后扫描、盘中实时扫描、后续回测使用同一套策略注册表。
    """
    ...
    is_hit, hit_strategy, info = evaluate_latest_signal(latest)

    if not is_hit or not info:
        return False, "", None

    # 兼容 realtime_strategy 原来的导出字段命名。
    if "收盘价" in info:
        info["最新价"] = info["收盘价"]
    if "今日涨跌幅" in info:
        info["涨跌幅"] = info["今日涨跌幅"]

    return True, hit_strategy, info
...
def scan_realtime_once(
    ...
    print_step_elapsed("1. 加载基础股票池", step_start_time)
    ...
    print_step_elapsed("2. 获取实时行情", step_start_time)

    step_start_time = time.time()
    rt_df = format_realtime_df(rt_raw_df)
    print_step_elapsed("3. 格式化实时行情", step_start_time)

    if rt_df.empty:
        print("实时行情为空，本轮扫描结束。")
        print_step_elapsed("本轮实时扫描总耗时", total_start_time)
        return pd.DataFrame()
    ...
    print_step_elapsed("4. 合并基础股票池信息", step_start_time)
    ...
    print()
    print_step_elapsed("5. 历史K线和日线策略扫描", scan_start_time)
    ...
    print_step_elapsed("10. 打印实时命中预览", step_start_time)
    print_step_elapsed("本轮实时扫描总耗时", total_start_time)
